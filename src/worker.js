...
async function buildXrayBestPingConfig(proxySettings, totalAddresses, chainProxy, outbounds, isFragment) {
    const remark = isFragment ? 'کانال تلگرام ما: t.me/bypassingsanction F - Best Ping 💥' : 'کانال تلگرام ما: t.me/bypassingsanction - Best Ping 💥';
...
async function buildXrayBestFragmentConfig(proxySettings, hostName, chainProxy, outbounds) {
    ...
    let config = buildXrayConfig(proxySettings, 'کانال تلگرام ما: t.me/bypassingsanction F - Best Fragment 😎', true, true, chainProxy, undefined, false);
...
async function buildXrayWorkerLessConfig(proxySettings) {
    let config = buildXrayConfig(proxySettings, 'کانال تلگرام ما: t.me/bypassingsanction F - WorkerLess ⭐', true, false, false, undefined, false);
...
async function getXrayWarpConfigs (proxySettings, warpConfigs, client) {
    ...
    for (const [index, endpoint] of warpEndpoints.split(',').entries()) {
        ...
        let warpConfig = buildXrayConfig(proxySettings, `کانال تلگرام ما: t.me/bypassingsanction ${index + 1} - Warp${proIndicator}🇮🇷`, false, false, false, undefined, true);
        let WoWConfig = buildXrayConfig(proxySettings, `کانال تلگرام ما: t.me/bypassingsanction ${index + 1} - WoW${proIndicator}🌍`, false, false, true, undefined, true);
    ...
    let xrayWarpBestPing = buildXrayConfig(proxySettings, `کانال تلگرام ما: t.me/bypassingsanction Warp${proIndicator}- Best Ping 🚀`, false, true, false, undefined, true);
    ...
    let xrayWoWBestPing = buildXrayConfig(proxySettings, `کانال تلگرام ما: t.me/bypassingsanction WoW${proIndicator}- Best Ping 🚀`, false, true, true, undefined, true);
...
async function getClashWarpConfig(proxySettings, warpConfigs) {
    ...
    selector.proxies = ['کانال تلگرام ما: t.me/bypassingsanction Warp - Best Ping 🚀', 'کانال تلگرام ما: t.me/bypassingsanction WoW - Best Ping 🚀'];
    warpUrlTest.name = 'کانال تلگرام ما: t.me/bypassingsanction Warp - Best Ping 🚀';
    ...
    WoWUrlTest.name = 'کانال تلگرام ما: t.me/bypassingsanction WoW - Best Ping 🚀';
    let warpRemarks = [], WoWRemarks = [];
    
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        const warpRemark = `کانال تلگرام ما: t.me/bypassingsanction ${index + 1} - Warp 🇮🇷`;
        const WoWRemark = `کانال تلگرام ما: t.me/bypassingsanction ${index + 1} - WoW 🌍`;
...
async function getClashNormalConfig (env, proxySettings, hostName) {
    ...
    selector.proxies = ['کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥'];
    urlTest.name = 'کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥';
...
async function getSingBoxWarpConfig (proxySettings, warpConfigs, client) {
    ...
    selector.outbounds = [`کانال تلگرام ما: t.me/bypassingsanction Warp${proIndicator}- Best Ping 🚀`, `کانال تلگرام ما: t.me/bypassingsanction WoW${proIndicator}- Best Ping 🚀`];
    config.outbounds.splice(2, 0, structuredClone(warpUrlTest));
    const WoWUrlTest = config.outbounds[2];
    warpUrlTest.tag = `کانال تلگرام ما: t.me/bypassingsanction Warp${proIndicator}- Best Ping 🚀`;
    warpUrlTest.interval = `${proxySettings.bestWarpInterval}s`;
    WoWUrlTest.tag = `کانال تلگرام ما: t.me/bypassingsanction WoW${proIndicator}- Best Ping 🚀`;
...
async function getSingBoxCustomConfig(env, proxySettings, hostName, client, isFragment) {
    ...
    selector.outbounds = ['کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥'];
    urlTest.interval = `${bestVLESSTrojanInterval}s`;
    urlTest.tag = 'کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥';
    const Addresses = await getConfigAddresses(hostName, cleanIPs, enableIPv6);
    const customCdnAddresses = customCdnAddrs ? customCdnAddrs.split(',') : [];
    const totalAddresses = [...Addresses, ...customCdnAddresses];
    const totalPorts = ports.filter(port => isFragment ? defaultHttpsPorts.includes(port) : true);
