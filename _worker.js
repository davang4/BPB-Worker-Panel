...
function generateRemark(index, port, address, cleanIPs, protocol, configType) {
    ...
    return `کانال تلگرام ما: t.me/bypassingsanction ${index} - ${protocol}${type} - ${addressType} : ${port}`;
}
...
async function renderHomePage (env, hostName, fragConfigs) {
	...
	<body>
		<h1>BPB Panel <span style="font-size: smaller;">${panelVersion}</span> کانال تلگرام ما: t.me/bypassingsanction</h1>
    ...
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        ...
        if (client === 'singbox' || client === 'hiddify') {
            let singboxOutbound = buildSingboxWarpOutbound(
                client === 'hiddify' ? `کانال تلگرام ما: t.me/bypassingsanction Warp Pro ${index + 1} 🇮🇷` : `کانال تلگرام ما: t.me/bypassingsanction Warp ${index + 1} 🇮🇷`, 
                warpIPv6, 
                privateKey, 
                publicKey, 
                endpoint,
        ...
        if (client === 'clash') {
            let clashOutbound = buildClashWarpOutbound(`کانال تلگرام ما: t.me/bypassingsanction Warp ${index + 1} 🇮🇷`, warpIPv6, privateKey, publicKey, endpoint, reserved, '');
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (client === 'singbox' || client === 'hiddify') {
                let singboxOutbound = buildSingboxWarpOutbound(
                    i === 1
                        ? `proxy-${index + 1}` 
                        : client === 'hiddify' 
                            ? `کانال تلگرام ما: t.me/bypassingsanction WoW Pro ${index + 1} 🌍` 
                            : `کانال تلگرام ما: t.me/bypassingsanction WoW ${index + 1} 🌍` , 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint, 
            ...
            if (client === 'clash') {
                let clashOutbound = buildClashWarpOutbound(
                    i === 1 ? `proxy-${index + 1}` : `کانال تلگرام ما: t.me/bypassingsanction WoW ${index + 1} 🌍`, 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint,
...
async function buildXrayWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, fragmentPackets, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = 'کانال تلگرام ما: t.me/bypassingsanction F - WorkerLess ⭐'
...
async function getXrayFragmentConfigs(env, hostName) {
    ...
    bestPing.remarks = 'کانال تلگرام ما: t.me/bypassingsanction F - Best Ping 💥';
    ...
    bestFragment.remarks = 'کانال تلگرام ما: t.me/bypassingsanction F - Best Fragment 😎';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? 'کانال تلگرام ما: t.me/bypassingsanction Warp Pro Best Ping 🚀' : 'کانال تلگرام ما: t.me/bypassingsanction Warp Best Ping 🚀';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `کانال تلگرام ما: t.me/bypassingsanction Warp Pro ${index + 1} 🇮🇷` : `کانال تلگرام ما: t.me/bypassingsanction Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag === 'chain') {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `کانال تلگرام ما: t.me/bypassingsanction WoW Pro ${proxyIndex} 🌍` : `کانال تلگرام ما: t.me/bypassingsanction WoW ${proxyIndex} 🌍`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? 'کانال تلگرام ما: t.me/bypassingsanction WoW Pro Best Ping 🚀' : 'کانال تلگرام ما: t.me/bypassingsanction WoW Best Ping 🚀';
...
function buildClashChainOutbound(chainProxyParams) {
    ...
    const { hostName, port, uuid, flow, security, type, sni, fp, alpn, pbk, sid, spx, headerType, host, path, authority, serviceName, mode } = chainProxyParams;
    let chainOutbound = {
        "name": "کانال تلگرام ما: t.me/bypassingsanction Chain Best Ping 💥",
        "type": "vless",
        "server": hostName,
        "port": +port,
        "udp": true,
        ...
        "flow": flow,
        "network": type,
        "dialer-proxy": "کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥"
    };
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    if (isWarp) {
        config['proxy-groups'][0].proxies = ['کانال تلگرام ما: t.me/bypassingsanction Warp Best Ping 🚀', 'کانال تلگرام ما: t.me/bypassingsanction WoW Best Ping 🚀'];
        config['proxy-groups'][1].name = 'کانال تلگرام ما: t.me/bypassingsanction Warp Best Ping 🚀';
        config['proxy-groups'][1].interval = +bestWarpInterval;
        config['proxy-groups'].splice(2, 0, structuredClone(config['proxy-groups'][1]));
        config['proxy-groups'][2].name = 'کانال تلگرام ما: t.me/bypassingsanction WoW Best Ping 🚀';
        ...
        config['proxy-groups'][0].proxies = ['کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥'];
        config['proxy-groups'][1].name = 'کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥';
...
async function getSingboxConfig (env, hostName, client, isWarp, isFragment) {
    ...
    if (isWarp) {
        ...
        config.outbounds[0].outbounds = client === 'hiddify'
            ? ["کانال تلگرام ما: t.me/bypassingsanction Warp Pro Best Ping 🚀", "کانال تلگرام ما: t.me/bypassingsanction WoW Pro Best Ping 🚀"]
            : ["کانال تلگرام ما: t.me/bypassingsanction Warp Best Ping 🚀", "کانال تلگرام ما: t.me/bypassingsanction WoW Best Ping 🚀"];
        config.outbounds.splice(2, 0, structuredClone(config.outbounds[1]));
        config.outbounds[1].tag = client === 'hiddify' 
            ? "کانال تلگرام ما: t.me/bypassingsanction Warp Pro Best Ping 🚀"
            : "کانال تلگرام ما: t.me/bypassingsanction Warp Best Ping 🚀";
        config.outbounds[2].tag = client === 'hiddify'
            ? "کانال تلگرام ما: t.me/bypassingsanction WoW Pro Best Ping 🚀"
            : "کانال تلگرام ما: t.me/bypassingsanction WoW Best Ping 🚀";
...
async function getNormalConfigs(env, hostName, client) {
    ...
    if (outProxy) {
        let chainRemark = `#${encodeURIComponent('کانال تلگرام ما: t.me/bypassingsanction Chain proxy 🔗')}`;
    ...
    outbounds: [
        {
            type: "selector",
            tag: "proxy",
            outbounds: ["کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥"]
        },
        {
            type: "urltest",
            tag: "کانال تلگرام ما: t.me/bypassingsanction Best Ping 💥",
            outbounds: [],
