...
const generateRemark = (index, port) => {
    ...
    switch (index) {
        case 0:
        case 1:
            remark = `@bypassingsanction - Domain_${index + 1} : ${port}`;
            break;
        case 2:
        case 3:
            remark = `@bypassingsanction - IPv4_${index - 1} : ${port}`;
            break;
        case 4:
        case 5:
            remark = `@bypassingsanction - IPv6_${index - 3} : ${port}`;
            break;
        default:
            remark = `@bypassingsanction - Clean IP_${index - 5} : ${port}`;
            break;
    }
...
const buildWorkerLessConfig = async (env, client) => {
    ...
    fragConfig.remarks  = '@bypassingsanction Frag - WorkerLess ⭐'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    bestPing.remarks = '@bypassingsanction Frag - Best Ping 💥';
    ...
    bestFragment.remarks = '@bypassingsanction Frag - Best Fragment 😎';
...
const getWarpConfigs = async (env, client) => {
    ...
    singboxWarpConfig.outbounds[0].outbounds = ['asad Warp Best Ping 🚀'];
    singboxWarpConfig.outbounds[1].tag = 'asad Warp Best Ping 🚀';
    ...
    xrayWarpBestPing.remarks = '@bypassingsanction - Warp Best Ping 🚀'
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: `@bypassingsanction - Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = `@bypassingsanction - WoW ${index/2 + 1} 🌍`;
...
const buildWoWOutbounds = async (remoteDNS, localDNS, blockAds, bypassIran, blockPorn, bypassLAN, wowEndpoint) => {
    ...
    wowEndpoint.split(',').forEach( (endpoint, index) => {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            singboxOutbound.tag = i === 1 ? `warp-ir_${index + 1}` : `asad WoW ${index + 1} 🌍`;    
...
const renderHomePage = async (env, hostName, fragConfigs) => {
            ...
            <tr>
                <td>
                    ${config.address === 'Best-Ping' 
                        ? `<div  style="justify-content: center;"><span><b>asad Best-Ping 💥</b></span></div>` 
                        : config.address === 'WorkerLess'
                            ? `<div  style="justify-content: center;"><span><b>asad WorkerLess ⭐</b></span></div>`
                            : config.address === 'Best-Fragment'
                                ? `<div  style="justify-content: center;"><span><b>asad Best-Fragment 😎</b></span></div>`
                                : config.address
                    }
                </td>
                <td>
	...
	<body>
		<h1>BPB Panel <span style="font-size: smaller;">${panelVersion}</span> asad</h1>
